...
fuel_nodes = nodes(j);        % nodes for each region
gap_nodes = nodes(j);
clad_nodes = nodes(j);
...
plot(cell2mat(r_sv(length(nodes))),T_a,'-o')
...
plot(cell2mat(r_sv(length(nodes))),q_a,'-o')
legend(legend_info,'Analytical Solution')
